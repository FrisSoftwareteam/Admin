    ...
    : BaseController(service, AuditLogSection.Forms)
{
        ...
        TForm model, string signatureString = null, IFormFile signatureFile = null) where TForm : IFormModel
    {
        try
        {
            ...
            if (formType == Forms.DataUpload && model is DataUpdateModel dataUpdateModel)
            {
                try
                {
                    await service.Email.SendDataUpdateEmailAsync(dataUpdateModel);
                }
                catch (Exception emailEx)
                {
                    logger.LogError(emailEx, "Data update notification email could not be sent");
                }
            }
