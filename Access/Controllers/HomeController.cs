...
public class HomeController : Controller
{
    ...
    {
        _logger.LogWarning($"HOME DEBUG: Index hit. IsAuthenticated={User.Identity.IsAuthenticated}, Name={User.Identity.Name}");
        try
        {
            ...
            _logger.LogWarning($"HOME DEBUG: User found: {user?.UserName}, Type={user?.Type}");
            ...
            return RedirectToAction("Index", user.Type.ToString());
        }
        catch (Exception ex)
        {
            _logger.LogError($"HOME DEBUG ERROR: {ex}");
        ...
        try
        {
            ...
            switch (user.Type)
            {
                case UserType.Shareholder:
                    if (!user.VisibleShareholders.Any(x => x.Verified && x.IsSubscribed))
    ...
    public IActionResult Error()
    {
        ...
        if (feature?.Error != null)
        {
            ...
            ViewBag.ErrorDetail = feature.Error.GetType().Name + ": " + feature.Error.Message;
            TempData["error"] ??= ViewBag.ErrorDetail;
        }
