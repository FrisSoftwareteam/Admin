...
public class DataService
{
    ...
    public DataService(string connectString)
    {
        ...
        Task.Run(async () =>
        {
            try { await EnsureObjects(); }
            catch (Exception ex) { Console.WriteLine($"[DataService] EnsureObjects failed: {ex.Message}"); }
        });
    }
    ...
    {
        using SqlConnection conn = new(_connectString);
        var shareholder = conn.Query<RegSH>($"SELECT * FROM ___RHoldings WHERE (RegCode = {regCode}) AND (AccountNo = {accountNo})").FirstOrDefault();
